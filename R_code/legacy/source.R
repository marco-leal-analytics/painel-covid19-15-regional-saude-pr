...
if (nrow(dadoscity) < 1) {numberofrow<-0  } else {
  ...
  viajem15<- function(cidade){
    if (as.character(cidade) == "MARINGA") {
      ...
      fig <- fig %>%  layout(hovermode = TRUE, spikedistance =  -1, barmode = 'stack',height= 460,
  ...
  viajem15<- function(cidade){
    ...
    fig <- plot_ly(viajem, x = ~ viajem$datas, y = ~ viajem$naoviajou, type = 'bar', name = 'Não Viajou', marker = list(color = 'rgb(8,48,107)'),height= 460)
...
taxaevol<-function(x){
  ...
  if (numberofrow < 10) {
    ...
    fig <- plot_ly(
      x = dadosfinal$time[10:nrow(dadosfinal)],height= 460,
      y = taxa,
      type = "scatter",
      mode = "lines+markers",
